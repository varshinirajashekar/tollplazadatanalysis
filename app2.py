...
def load_data(path):
    ...
    for i, m in enumerate(months):
        temp = df_raw[
            ["Fee Plaza Name", "State", count_cols[i], amount_cols[i]]
        ].copy()
...
monthly_stats = (
    df_filtered.groupby("Month")[["Transactions", "Revenue"]]
    .sum()
    .reset_index()
)
...
# ---------------- DENSITY PLOT ----------------
st.divider()
monthly_dist = monthly_stats["Revenue"]

fig5, ax5 = plt.subplots()
monthly_dist.plot(kind="kde", ax=ax5)
ax5.set_title("Density Plot of Monthly Revenue")
st.pyplot(fig5)

# ---------------- SCATTER + CORRELATION ----------------
st.divider()
fig6, ax6 = plt.subplots()
ax6.scatter(monthly_stats["Transactions"], monthly_stats["Revenue"])
ax6.set_xlabel("Transactions")
